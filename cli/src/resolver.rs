...
impl Resolver {
        ...
        if self.config.providers_order.is_empty() && self.config.profile.is_provider_allowed(ProviderType::LlmsTxt) && self.config.profile.is_provider_allowed(ProviderType::Jina) {
            ...
            if let Ok((mut res, pt)) = parallel_result {
                if res.is_valid(self.config.min_chars) {
                    ...
                    self.post_process_result(&mut res, &mut metrics).await;
                    return Ok(res);
                }
            }
        }
        ...
        for provider_type in providers_order {
            ...
            if let Ok(results) = results {
                ...
                if !results.is_empty() {
                    ...
                    self.post_process_result(&mut first, &mut metrics).await;
                    if let Some(cache) = &self.cache {
    ...
    async fn extract_with_provider(
        ...
        // Automatic routing for specialized document/image formats
        if provider_type == ProviderType::DirectFetch {
            if (url.ends_with(".pdf") || url.ends_with(".docx") || url.ends_with(".pptx")) && self.docling.is_available() {
                return self.docling.extract(url).await;
            }
            if (url.ends_with(".png") || url.ends_with(".jpg") || url.ends_with(".jpeg")) && self.ocr.is_available() {
                return self.ocr.extract(url).await;
            }
        }
    ...
    /// Post-process result (link validation, scoring, compaction)
    async fn post_process_result(&self, res: &mut ResolvedResult, metrics: &mut ResolveMetrics) {
        ...
        res.metrics = Some(metrics.clone());
    }
