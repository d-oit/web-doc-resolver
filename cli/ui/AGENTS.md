...
|---|---|---|
| **Production** | `https://web-eight-ivory-29.vercel.app` | Vercel native GitHub integration (auto-deploy on push to `main`) |
...
The `BASE_URL` env var overrides the default `https://web-eight-ivory-29.vercel.app` in `playwright.config.ts`. E2E tests must pass before any release is published.
